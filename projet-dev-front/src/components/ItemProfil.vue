  ...
  <div class="item-container">
    <div class="info">
      ...
      <div class="right-side">
        ...
        <button
          v-if="userInfo.userType == 1 && item.quantityCart < 1"
          v-on:click="addToCart(item.id)"
          style="display: flex; justify-content: center; align-items: center"
        >
          Add To Cart<v-icon name="bi-cart" />
        </button>
        <div
        ...
        <div style="display: flex; flex-direction: row; align-items: center">
          <button
            ...
            "
          >
            <v-icon name="bi-trash" scale="3.0" />
          </button>
          <button
            v-if="userInfo.userType == 2"
            v-on:click="deleteItem(item.id)"
            ...
            "
          >
            <v-icon name="bi-wrench-adjustable" scale="3.0" />
          </button>
        </div>
      </div>
    </div>
    <div class="randItem">
      <div class="card" v-for="item in this.itemsCat" v-bind:key="item.id">
        ...
        <div class="button-section">
          ...
          <button
            v-if="userInfo.userType == 1 && item.quantityCart <= 0"
            v-on:click="addToCart(item.id)"
          >
            Add To Cart
          </button>
...
export default {
  ...
  methods: {
    ...
    async goToItemPage(idItem) {
      await this.$router.push({ path: "/itemProfil", query: { id: idItem } });
      await this.initPage();
    },
    async getInfoItem() {
    ...
    async getCategory() {
      const req = await axios.get(
        "http://localhost/category/" + this.item.categoryID
      );
      const res = await req.data;
    ...
    async getRandomCatItems() {
      const req = await axios.get(
        "http://localhost/items/randomCategory/" + this.item.categoryID
      );
      const res = await req.data;
      await res.forEach((itemGet, index) => {
        if (itemGet.id == this.item.id) res.splice(index, 1);
      });
      this.itemsCat = await res;
    },
    async getUserInfo() {
      const req = await axios.get(
        "http://localhost/users/" + localStorage.getItem("userID")
      );
      const res = await req.data;
